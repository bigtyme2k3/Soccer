...
SPORT = os.environ["SPORT_KEY"]
LEAGUE = os.getenv("LEAGUE_SLUG", SPORT.replace("soccer_", ""))
...
def main():
    ...
    raw = {"fetched_at": now.isoformat(), "sport": SPORT, "league": LEAGUE,
           "events": events, "event_markets": []}
    ...
    raw_path = OUT / f"{LEAGUE}_raw_{stamp}.json"
    ...
    for item in raw["event_markets"]:
        ...
        if not isinstance(data, dict):
            continue
        base = {"event_id":data.get("id"),"commence_time":data.get("commence_time"),
    ...
    latest = {"fetched_at":now.isoformat(),"sport":SPORT,"league":LEAGUE,
              "bookmakers":BOOKS.split(","),"rows":rows}
    (OUT / f"{LEAGUE}_latest.json").write_text(json.dumps(latest, indent=2), encoding="utf-8")
    print(f"league={LEAGUE} events={len(events)} normalized_rows={len(rows)} raw={raw_path}")
